...
int run_length_decode(int *code, int size, int *msg)
{
	...
	int msg_index  = 0; // iterates through msg array.
	int code_index; // iterates through code array.
	for(code_index = 0; code_index < size; code_index++) {
		if(code[code_index] == 0) { // if a zero is found in the array
			code_index++;
			//this adds in the number of 1's into the msg array
			while(count < code[code_index]) {
				msg[msg_index] = 1;
				msg_index++;
				count++;
			}

		} else { // else add the element into the array
			msg[msg_index] = code[code_index];
			msg_index++;
		}
		count = 0;
	}
...
int run_length_encode(int *msg, int size, int *code)
{
	...
	int index = 0;
	int code_index = 0;
	while(index < size) { //iterates through msg array
		// if 3 repeating 1's are present
		if(msg[index] == 1 && msg[index+1] == 1 && msg[index+2] == 1) {
			index += 2;
			count = 3;
			//increment the count
			while(msg[index+1] == 1) {
			...
			//store the number of occurences of 1's with a zero
			code[code_index] = 0;
			code[code_index+1] = count;
			code_index += 2;
			count = 3;

		} else {// else just fill in the code array with the current msg index
...
int check_run_length(int *msg, int size, int *code)
{
	...
	//if not same then equate to false
